...
import glob
import subprocess
setting_screenout = subprocess.check_output("export DISPLAY=:0",shell=True) # Setting the screen output permission data 
...
mode = 0o777 # Setting the permission mode for the directory created in the system 
try:
  ...
  os.mkdir("Packagelibrary",mode)
  os.mkdir("ComponentDoc",mode) # Activate the permission 
  os.mkdir("tempolarydocextract",mode) # Activate the permission
  os.mkdir("Configuresearch",mode) # Activate the permission
  os.mkdir("Packagelibrary",mode) #Activate permission for the package library directory
except: 
...
PINSFUNC = EXTRACT+"/"+inputcomp+"/"+inputcomp+"R"+"Pinsfunc"
                                             ...
                                             if len(listcheck) != 1:
                                                  ...
                                                  if intersecfile != []:
                                                     for ih in range(0,len(intersecfile)):
                                                           shutil.copy2(Readingdir+"/"+intersecfile[ih],Pathdata+"/"+findnewdir[il]) #Get the documentation of the components 
                                             ...
                                             if len(listcheck) != 1:        
                                                for ihl in range(0,len(Basedir)): 
                                                      ...
                                                      if intersecfile != []:
                                                            for ih in range(0,len(intersecfile)):
                                                                 print("Position",ih)
                                                                 shutil.copy2(Readingdir2+"/"+intersecfile[ih],Pathdata+"/"+findnewdir[il]) # Get the documentation of the components 
...
def Paringtables(inputcomp,Predictbreak):  #Bredict the breaking data 
        ...
        try:
            ...
            print("Dicrectory was already created")
...
def Packagefor_3dlibclass(input1,inputcomp,Pathdata,listConfig):
       ...
       try:
          ...
          os.mkdir(EXTRACT+"/"+inputcomp+"/"+inputcomp+"M"+"Pinsfunc",mode)
       except: 
          print("Directory was already created")
       PINSMERFUNC = EXTRACT+"/"+inputcomp+"/"+inputcomp+"M"+"Pinsfunc"  #Getting the new path for the classified page pdf data from the extracted file csv 
       ...
       if datalist != []: #Checking if the directory is not blank  
           ...
           for il in range(0,len(listrecorder)):
                       ...
                       if checkmatch !=[]:
                              ...
                              if percent.ratio()*100 >= 85.71428571428571 and percent.ratio()*100 <= 100.0: 
                                       ...
                                       #os.system("sudo cp "+EXTRACT+"/"+inputcomp+"/"+inputcomp+"_"+str(getpage)+".csv -t ",PINSFUNC)
                                       ...
                                       #Getting the header index for classifying the type of the merging mode
           ...
           #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
              ...
              #New Mergesorting the the csv file for thee eash case 
              #Case 1 NO. detected for the single package but have multiple page detected               
              print("Starting merging the csv file....") # Merging the csv file for the devices pins function
              listreadymerge = os.listdir(PINSFUNC) #Getting the directory in the path 
              print("Running the directory for merging data:",listreadymerge) #Getting the ready merge list for running the merging data
              for ir in range(0,len(listreadymerge)): 
                   print("Getting the index header premerge list:",Deviceindexheader) 
                   if len(Deviceindexheader) == 1: 
                         if Deviceindexheader[0] == "NO.":          
                            print("Check list file ready to merge",listreadymerge[ir])
                            print("Checking the lenge of the page csv data:",len(listreadymerge)) #Getting the lenght of the csv file in the extracted directory
                            # Working on the merging loop condition here 
                           
                   #Case 2 Multiple package name multiple page using the package name classification 
                   if len(Deviceindexheader) >= 2:
                        if Deviceindexheader[0] == "NO.":          
                           print("Check list file ready to merge",listreadymerge[ir])
                           # Working on the looping function of the index classify 
                   #Case 3 Sorting by modulo page 
                   if len(Deviceindexheader) >= 2:
                        if Deviceindexheader[0] == "NO.":          
                           print("Check list file ready to merge",listreadymerge[ir])
                           # Working on the looping function of the index classify 
              for ir in range(0,len(sorting2)):
                     ...
                     df = pd.read_csv(PINSFUNC+"/"+inputcomp+"_"+str(sorting2[ir])+".csv") #Getting the data frame from the list csv file
                     print(df) #Getting the dataframe of the csv file 
                     listdataindex = list(df.columns.values) 
                     print("Reading header columns for matching check:",listdataindex) #getting the data index 
                     print("Begin recording data of header")
                     recordheader2[str(sorting2[ir])] = listdataindex #Record data frame
              print("Recorded data2 from the header columns")
              print("Record data2 function",recordheader2)      
           #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
           #Matching sequence 
           listrecorder2 = list(recordheader2) 
           data = open(CONFIG+"/"+str(listConfig),'r') #Open the configuretion file for the path 
           datas = data.readline()
           transfer = json.loads(datas)
           for il in range(0,len(listrecorder2)):
                       listheader =  recordheader.get(listrecorder2[il]) 
                       print(listheader) #Getting the list header recorded
                       gettopic = transfer.get("Pin connection function").get("Pin functions")
                       #print("Getting the topic header",gettopic)
                       checkmatch = intersection(gettopic,listheader) #Getting the topic check list 
                       ...
                       if checkpackage !=[]: 
                              ...
                              if percent.ratio()*100 >= 85.71428571428571 and percent.ratio()*100 <= 100.0: 
                                       ...
                                       # Setting the configuretion for the leng with of the data under lot of number of package detected in the same time 
                                       #Using the conditional value of the Deviceindexheader for the criteria on the processing separation
                                       if Deviceindexheader[0] != "NO.":
...
Packagefor_3dlibclass(input1,inputcomp,Pathdata,listConfig[0]) #Getting the dataframe
